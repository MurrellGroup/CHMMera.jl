...
function get_recombination_events(query::Vector{Int64}, references::Vector{Vector{Int64}}; fast::Bool = true, prior_probability::Float64 = 1/300, mutation_rates = [0.02, 0.04, 0.07, 0.11, 0.15])
    if fast
        hmm = ApproximateHMM(vovtomatrix(references), 0.05, prior_probability)
    else
        hmm = FullHMM(vovtomatrix(references), mutation_rates, prior_probability)
    end
...
"""
function get_log_site_probabilities(query::String, references::Vector{String}; fast = true, prior_probability = 1/300)
    recombs = get_recombination_events(query, references, fast=fast) # defaults to fast=true
