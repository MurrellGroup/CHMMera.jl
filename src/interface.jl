...
function get_recombination_events(queries::Vector{Vector{UInt8}}, references::Vector{Vector{UInt8}}, bw::Bool = true, mutation_probabilities::Vector{Float64} = [0.02, 0.04, 0.07, 0.11, 0.15], base_mutation_probability::Float64 = 0.05, prior_probability::Float64 = 1/300, details::Bool = false)
...
"""
get_recombination_events(queries::Vector{String}, references::Vector{String}, bw::Bool = true, mutation_probabilities::Vector{Float64} = [0.02, 0.04, 0.07, 0.11, 0.15], base_mutation_probability::Float64 = 0.05, prior_probability::Float64 = 1/300, detailed::Bool = false) =
...
function get_log_site_probabilities(queries::Vector{String}, references::Vector{String}, bw::Bool = true, mutation_probabilities::Vector{Float64} = [0.02, 0.04, 0.07, 0.11, 0.15], base_mutation_probability::Float64 = 0.05, prior_probability::Float64 = 1/300)
    ...
    recombs = get_recombination_events(queries, references, bw, mutation_probabilities, base_mutation_probability, prior_probability, false)
    ...
    Threads.@threads for i in eachindex(recombs)
        logsiteprobabilities_[i] = logsiteprobabilities(recombs[i].recombinations, queries[i], hmm, copy(mutation_probabilities))
    end
