...
function get_recombination_events(queries::Vector{Vector{Int64}}, references::Vector{Vector{Int64}}; fast::Bool = true, mutation_probabilities = [0.02, 0.04, 0.07, 0.11, 0.15], base_mutation_probability = 0.05, prior_probability::Float64 = 1/300, startingpoint = false)
    
    hmm = fast ? ApproximateHMM(vovtomatrix(references), base_mutation_probability, prior_probability) :  FullHMM(vovtomatrix(references), mutation_probabilities, prior_probability)
    recomb_f(query::Vector{Int64}, hmm::HMM, mutation_probabilities::Vector{Float64}) = startingpoint ? findrecombinations_and_startingpoint(query, hmm, mutation_probabilities) : findrecombinations(query, hmm, mutation_probabilities)
...
"""
get_recombination_events(queries::Vector{String}, references::Vector{String}; fast::Bool = true, mutation_probabilities = [0.02, 0.04, 0.07, 0.11, 0.15], base_mutation_probability = 0.05, prior_probability::Float64 = 1/300, startingpoint = false) = 
    get_recombination_events(as_ints.(queries), as_ints.(references), fast = fast, mutation_probabilities = mutation_probabilities, base_mutation_probability = base_mutation_probability, prior_probability = prior_probability, startingpoint = startingpoint)
